        ...
        {submitError && (
          ...
          >
            <motion.div
              ...
              exit={{ scale: 0.9, opacity: 0 }}
              onClick={(e) => e.stopPropagation()}
              className="bg-black border border-red-700 rounded-lg p-8 max-w-md w-full"
            >
              <div className="flex flex-col items-center text-center">
                <div className="w-16 h-16 bg-red-500/20 rounded-full flex items-center justify-center mb-4">
                  <AlertCircle size={32} className="text-red-500" />
                </div>
                <h3 className={`${lexendDeca.className} text-2xl font-medium mb-2 text-red-400`}>
                  Submission Failed
                </h3>
                <p className="text-gray-400 mb-6">{submitError}</p>
                <button
                  onClick={() => setSubmitError(null)}
                ...
                {formData.propertyPhotos.map((photo, index) => (
                  <div key={index} className="relative group">
                    ...
                    <div className="mt-2 px-2">
                      <p className="text-sm text-gray-400 truncate" title={photo.name}>
                        {photo.name}
                      </p>
                      <p className="text-xs text-gray-500">
